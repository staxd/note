...
function sendAjax(obj, callback) {
	var _defaults = {
		...
		//config
		baseURL: obj.baseURL || 'http://www.joinfit.live:8092/sandbagapp/',
		// baseURL: obj.baseURL||'http://47.99.81.5:8092/sandbagapp/',
		headers: {
	...
	const bfcallback = callback.before || function (data) { };
	const comcallback = callback.coment || function (data) { };

	bfcallback()
	axios(_defaults).then(function (res) {

		if (res.data.code == 200 || res.data.code == "OK") {
			comcallback(res)
			scallback(res.data)
		} else {
